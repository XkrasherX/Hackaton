...
### Features
The system uses LLM (Large Language Model) from Groq for automatic flight analysis:
...
**Important**: The system works without Groq API! If the API key is unavailable, automatic fallback mode is enabled.

### Installation (OPTIONAL)

For LLM functions, install Groq:
...
- **Mixtral-8x7B**: Fast, accurate, expert-level
- **Free tier**: 30 API requests per minute
- **No ads**: No limits on request size
...
- [x] **AI Assistant** for automatic anomaly detection and recommendations (DONE)
- [x] **Enhanced Streamlit Dashboard** with progress, charts, and statistics (DONE)
- [x] **Theoretical Justification** for quaternions, integration, coordinates (DONE)
- [ ] Sensor fusion (Kalman filter) for improved accuracy
- [ ] Support for other log formats (Ardupilot CSV, PX4 ULog)
- [ ] Detailed flight phase statistics
- [ ] Multi-log comparison (benchmark)
- [ ] KML export for Google Earth
- [ ] Web version without local installation
